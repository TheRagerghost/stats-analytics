...
class Univariate:
    ...
    def __str__(self) -> str:
        strings = []
        strings.append(f"Input: {self.values}")
        strings.append(f"Sorted: {self.n_sorted}")
        strings.append(f"\n")

        strings.append(f"It consists of {self.i_count()} intervals with size {self.i_size()}.")
        strings.append(f"Intervals: {self.get_intervals()}")
        strings.append(f"Values split by intervals: {self.split_data[0]}")
        strings.append(f"Distribution values: {self.split_data[2]}")
        strings.append(f"Cumulative distribution: {self.split_data[3]}")
        strings.append(f"Frequency values: {self.split_data[4]}")
        strings.append(f"Cumulative frequency: {self.split_data[5]}")
        strings.append(f"Mean values: {self.split_data[6]}")
        strings.append(f"\n")

        strings.append(f"Mean: {self.mean()}")
        mode, mint = self.mode()
        strings.append(f"Mode of interval({mint}): {mode}")
        strings.append(f"Median: {self.median()}")
        strings.append(f"Range: {self.range()}")
        strings.append(f"Variance: {self.variance()}")
        strings.append(f"Standard deviation: {self.stdev()}")
        cv, scv = self.cov()
        strings.append(f"Coefficient of variance ({scv}): {show_as_percentage(cv)}")
        strings.append(f"Standard error of the mean: {self.sem()}")
        skew, ness = self.skewness()
        strings.append(f"Skewness ({ness}): {skew}")
        kurt, osis = self.kurtosis()
        strings.append(f"Kurtosis ({osis}): {kurt}")
        output = "\n".join(strings)
        return f"===\n\n{output}\n"
...
class Bivariate:
    ...
    def __str__(self):
        strings = []
        strings.append(f"Input X ({self.sums[0]} total, {self.means[0]} mean): {self.values[0]}")
        strings.append(f"Input Y ({self.sums[1]} total, {self.means[1]} mean): {self.values[1]}")
        strings.append(f"\n")

        strings.append(f"=== First method: ===")
        strings.append(f"XMD (x - mean): {self.data_a[0]}")
        strings.append(f"YMD (y - mean): {self.data_a[1]}")
        strings.append(f"XMD*YMD ({self.sums_a[0]} total): {self.data_a[2]}")
        strings.append(f"XMD^2 ({self.sums_a[1]} total): {self.data_a[3]}")
        strings.append(f"YMD^2 ({self.sums_a[2]} total): {self.data_a[4]}")
        strings.append(f"\n")

        xssd, yssd = self.sample_standard_deviation()
        strings.append(f"Sample standard deviation X: {xssd}, Y: {yssd}")
        strings.append(f"Pearson correlation coefficient: {self.pearson_correlation_coefficient()}")
        strings.append(f"Determination coefficient: {show_as_percentage(self.determination_cofficient())}")
        strings.append(f"\n")

        strings.append(f"=== Second method: ===")
        strings.append(f"X^2 ({self.sums_b[0]} total): {self.data_b[0]}")
        strings.append(f"Y^2 ({self.sums_b[1]} total): {self.data_b[1]}")
        strings.append(f"X*Y ({self.sums_b[2]} total): {self.data_b[2]}")
        xdisp, ydisp = self.dispersion()
        strings.append(f"Dispersion X:{xdisp} Y:{ydisp}")
        strings.append(f"Sample correlation momentum: {self.sample_correlation_momentum()}")
        strings.append(f"Sample correlation coefficient: {self.sample_correlation_coefficient()}")
        strings.append(f"Line of regression (x = 10..20): {self.line_of_regression([x for x in range(10, 21, 1)])}")
        output = "\n".join(strings)
        return f"===\n\n{output}\n"
